...
export const getRange = ({
  ...
  const { boundaryCount = 1, siblingCount = 1 } = rangeOptions;
  ...
  const siblingsStartPage = Math.max(
    Math.min(page - siblingCount, pageCount - boundaryCount - siblingCount * 2 - 1),
    boundaryCount + 2,
  );

  const siblingsEndPage = Math.min(
    Math.max(page + siblingCount, boundaryCount + siblingCount * 2 + 2),
    endRange.length > 0 ? endRange[0] - 2 : pageCount - 1,
  );

  const centerRange = range(siblingsStartPage, siblingsEndPage);

  const result = [
    ...startRange,
    ...(siblingsStartPage > boundaryCount + 2
      ? [START_ELLIPSIS]
      : boundaryCount + 1 < pageCount - boundaryCount
      ? [boundaryCount + 1]
      : []),
    ...centerRange,
    ...(siblingsEndPage < pageCount - boundaryCount - 1
      ? [END_ELLIPSIS]
